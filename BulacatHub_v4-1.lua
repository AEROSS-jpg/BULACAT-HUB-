...
local CFG = {
    ...
    WALK_SPEED    = 80,
    HOLD_HEIGHT   = 350,
    ALL_MAP_RANGE = 99999, -- all map scan range
    PARTICLE_COUNT= 28,
}
...
-- // ─────────────────────────────────────────────
-- // REAL PET PREDICT (reads actual StringValues/TextLabels/Model names inside egg)
-- // ─────────────────────────────────────────────
...
local function predictRarity(egg)
    local src = egg.Name:lower()
    for _,v in ipairs(egg:GetDescendants()) do
        if v:IsA("StringValue") or v:IsA("TextLabel") or v:IsA("StringValue") then
            local t = ""
            if v:IsA("StringValue") then t = (v.Value or ""):lower()
            elseif v:IsA("TextLabel") then t = (v.Text or ""):lower() end
            src = src .. " " .. t .. " " .. v.Name:lower()
        end
    end
    -- also check parent model name
    if egg.Parent and egg.Parent:IsA("Model") then
        src = src .. " " .. egg.Parent.Name:lower()
    end
    -- rank order: divine first
    local order = {"divine","eternal","secret","cosmic","legendary","epic","rare","uncommon"}
    local mapped = {"Divine","Eternal","Secret","Cosmic","Legendary","Epic","Rare","Uncommon"}
    for i,rk in ipairs(order) do
        for _,kw in ipairs(RARITY_KEYWORDS[rk]) do
            if src:find(kw) then return mapped[i] end
        end
    end
    return "Common"
end
...
local function predictPet(egg)
    -- Step 1: scan StringValues with pet-like names
    for _,v in ipairs(egg:GetDescendants()) do
        if v:IsA("StringValue") then
            local nm = v.Name:lower()
            for _,k in ipairs(PET_KEYWORDS) do
                if nm:find(k) and v.Value ~= "" and #v.Value < 60 then
                    return v.Value
                end
            end
        end
    end
    -- Step 2: scan TextLabels that look like pet names (short, not empty, not pure numbers)
    for _,v in ipairs(egg:GetDescendants()) do
        if v:IsA("TextLabel") and v.Text ~= "" and #v.Text < 50 then
            local t = v.Text
            if not t:match("^%d+$") and not t:lower():find("egg") and not t:lower():find("rarity") then
                return t
            end
        end
    end
    -- Step 3: check model name for pet hint
    if egg.Parent and egg.Parent:IsA("Model") then
        local mn = egg.Parent.Name
        if not mn:lower():find("egg") and #mn < 40 then
            return mn
        end
    end
    -- Step 4: use egg name stripping "Egg" word
...
-- // ─────────────────────────────────────────────
-- // ALL-MAP EGG SCAN (no distance limit, rarest first)
-- // ─────────────────────────────────────────────
...
local function enableGodMode()
    ...
    -- all parts: no collision so attacks and knockbacks pass through
...
-- // ─────────────────────────────────────────────
-- // MONSTER RIDE (Miranda style — weld to nearest rideable creature/mount)
-- // ─────────────────────────────────────────────
local rideWeld = nil
...
-- // ─────────────────────────────────────────────
-- // STEAL ENGINE (unknockable, fly high, interact, then ride monster)
-- // ─────────────────────────────────────────────
local function stealEgg(egg, onDone)
    ...
    flyTo(highPos, CFG.FLY_SPEED, function()
        ...
        -- proximity prompt search (egg and parent)
        ...
        -- TRY MONSTER RIDE (Miranda style)
        local rode = rideNearestMonster()
        if not rode then
            -- fallback: speed boost
            local hum = getHuman()
...
-- Particle container (orbiting dots)
...
-- Animate particles on heartbeat
local t0 = tick()
...
do
    ...
    -- toggle main on click (if not dragging much)
    local downPos
    logoBtn.MouseButton1Down:Connect(function() downPos = UserInputService:GetMouseLocation() end)
    logoBtn.MouseButton1Up:Connect(function()
        local cur = UserInputService:GetMouseLocation()
        if downPos and (cur - downPos).Magnitude < 6 then
            -- toggle main panel (handled below via MainFrame reference)
        end
    end)
end
...
mk("TextLabel",{Size=UDim2.new(0,220,0,16),Position=UDim2.new(0,54,0,32),BackgroundTransparency=1,Text="Egg Stealer v4 — All Map",TextColor3=C.subtext,TextSize=11,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=12},Header)
...
corner(6,VerBadge)
mk("TextLabel",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="v4.0",TextColor3=Color3.new(1,1,1),TextSize=10,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=13},VerBadge)
...
local StatusTxt = mk("TextLabel",{Size=UDim2.new(1,-32,1,0),Position=UDim2.new(0,28,0,0),BackgroundTransparency=1,Text="Idle — BULACAT HUB v4 ready",TextColor3=C.text,TextSize=12,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=12},StatusCard)
...
local _,getOnceOn,setOnceOn = makeToggle(CtrlCard,215,"Steal Once",C.gold)

-- // EGG LIST (scrollable, all-map)
...
-- // AUTO STEAL LOOP (smooth, no lag spike)
...
print("🐱 BULACAT HUB v4 — Click the particle logo!")
