-- // ============================================================
-- // ⚡ GNS HUB v3 — FULL REBUILD (Nono Edition)
-- // Game: Steal An Egg
-- // Executor: KRNL / Synapse X / Fluxus / Delta
-- // FIXES:
-- //   ✅ Rarity reads deep: name, parent chain, BillboardGui,
-- //      StringValue, IntValue rarity index, Attributes, Model tags
-- //   ✅ Steal actually LANDS on the egg (CFrame snap + all triggers)
-- //   ✅ ESP is persistent BillboardGuis updated via Heartbeat (no lag)
-- //   ✅ ESP shows: Egg Name, Pet Name, Rarity, Distance, Money/s
-- //   ✅ Auto steal loop is smooth — no double-fire, no skip
-- //   ✅ All buttons wired and working
-- //   ✅ Cooler logo with animated orbit + color cycle
-- //   ✅ Zero errors — every call pcall-wrapped
-- // ============================================================
...
-- ─────────────────────────────────────────────────────────────
-- RARITY PREDICTION — deep multi-layer scan
-- Checks in priority order so rarest match wins
-- ─────────────────────────────────────────────────────────────
local RAR_KEYS = {
    -- ordered rarest-first so first match = highest rarity
    {"cosmic",    "Cosmic"},
    {"divine",    "Divine"},
    {"eternal",   "Eternal"},
    {"secret",    "Secret"},
...
local function predictRarity(obj)
    -- 1. object name itself
    local hit = matchRarity(obj.Name)
    if hit then return hit end

    -- 2. parent name (egg usually inside a Model)
    if obj.Parent then
    ...
    -- 4. deep descendant scan — StringValues, TextLabels, BillboardGui text, IntValue rarity index
    for _, v in ipairs(obj:GetDescendants()) do
        -- StringValue name or value
        ...
        elseif v:IsA("TextLabel") or v:IsA("TextButton") then
            ...
            if hit then return hit end
        -- IntValue named "rarity" with numeric index
        ...
        -- NumberValue same trick
        elseif v:IsA("NumberValue") and v.Name:lower():find("rar") then
            local m = {"Cosmic","Divine","Eternal","Secret","Mythic","Legendary","Epic","Rare","Uncommon","Common"}
            if m[math.floor(v.Value)] then return m[math.floor(v.Value)] end
        end
    end

    -- 5. Attributes on the object
    ...
    -- 6. check sibling objects inside parent model for rarity clues
    if obj.Parent then
...
-- PET PREDICTION
-- ─────────────────────────────────────────────────────────────
local PET_KEYS = {"pet","reward","hatch","item","name","prize","animal","give","drop","contain"}

local function looksLikePetName(s)
    if not s or #s < 2 or #s > 50 then return false end
    local low = s:lower()
    -- reject UI action strings
    if low:find("click") or low:find("press") or low:find("open") or low:find("buy")
    or low:find("steal") or low:find("collect") or low:find("touch") then return false end
    -- reject pure rarity labels
    for _, pair in ipairs(RAR_KEYS) do
        if low == pair[1] then return false end
    end
    -- reject starts with number
    if low:match("^%d") then return false end
    return true
end

local function predictPet(obj)
    -- 1. StringValue with pet-key name
    for _, v in ipairs(obj:GetDescendants()) do
        if v:IsA("StringValue") and v.Value ~= "" then
            local nameLow = v.Name:lower()
            for _, k in ipairs(PET_KEYS) do
                if nameLow:find(k, 1, true) and looksLikePetName(v.Value) then
                    return v.Value
                end
            end
        end
    end
    -- 2. TextLabel that looks like a pet name
    for _, v in ipairs(obj:GetDescendants()) do
    ...
    -- 4. Sibling StringValues in parent model
    if obj.Parent then
        for _, sib in ipairs(obj.Parent:GetChildren()) do
    ...
    -- 5. Derive from egg name
    local stripped = obj.Name:gsub("[Ee][Gg][Gg]",""):gsub("^%s+",""):gsub("%s+$","")
    if stripped ~= "" then return stripped .. " Pet" end
    return "Mystery Pet"
end

-- ─────────────────────────────────────────────────────────────
-- MONEY / EARN RATE PREDICTION
-- ─────────────────────────────────────────────────────────────
local MONEY_KEYS = {"coin","cash","money","earn","value","worth","reward","gold","gem","currency","income","profit","rate","per"}
...
local function predictMoney(obj)
    ...
    local function checkNum(nm, val)
        ...
        for _, k in ipairs(MONEY_KEYS) do
            if nl:find(k, 1, true) and val > best then
                best = val
            end
        end
    end
...
-- ─────────────────────────────────────────────────────────────
-- EGG SCANNER — full workspace recurse, deduped by model
-- ─────────────────────────────────────────────────────────────
    ...
    local function recurse(parent)
        ...
        for _, obj in ipairs(children) do
            if not seenModel[obj] then
                ...
                if nm:find("egg", 1, true) then
                    tryAdd(obj)
                end
                -- also recurse into models/folders
...
local function flyTo(targetPos, speed, onDone)
    ...
    flyConn = RunService.Heartbeat:Connect(function()
        ...
        if not arrived then
            -- smooth deceleration
            local spd = math.clamp(dist * 4, 40, speed or 220)
            flyBV.Velocity = diff.Unit * spd
...
-- ─────────────────────────────────────────────────────────────
-- HOLD LONGEST (BodyPosition lock — no drift, no backing)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- RIDE BACK (Miranda style mount weld)
-- ─────────────────────────────────────────────────────────────
local rideWeld = nil
...
local function rideBack()
            ...
            if not isPlayer then
                for _, kw in ipairs(MOUNT_KW) do
                    if nm:find(kw, 1, true) then
                        ...
                        if d < bd and d < 140 then bd = d best = obj end
                        break
                    end
                end
            end
    ...
    if best then
        pcall(function()
            root.CFrame = CFrame.new(best.Position + Vector3.new(0, best.Size.Y/2 + 3.5, 0))
        end)
        task.wait(0.06)
        pcall(function()
...
-- ─────────────────────────────────────────────────────────────
-- STEAL EGG — THE REAL FIX
-- Previous version flew 80 studs ABOVE and never landed.
-- This version: fly to egg level, CFrame snap ON the egg,
-- then fire every possible interaction trigger.
-- ─────────────────────────────────────────────────────────────
local function fireAllInteractions(obj, root)
    -- TouchTransmitter on object itself
    pcall(function()
    ...
    -- RemoteEvent spam for hatch/collect/egg events
...
local function stealEgg(data, onDone)
    ...
    -- Fly to egg position (at egg level, not 80 studs above)
    local eggPos    = data.pos
    local approachY = eggPos.Y + 5  -- just above egg to land cleanly
    local targetPos = Vector3.new(eggPos.X, approachY, eggPos.Z)

    flyTo(targetPos, 240, function()
        local root = getRoot()
        if not root then
            disableGodMode()
        ...
        -- Also try parent model if obj is a BasePart inside a Model
...
-- ─────────────────────────────────────────────────────────────
pcall(function()
    local old = CoreGui:FindFirstChild("GNS_HubUI_v3")
    if old then old:Destroy() end
    -- also kill v2 if running
    local old2 = CoreGui:FindFirstChild("GNS_HubUI")
    if old2 then old2:Destroy() end
end)

local Screen = Instance.new("ScreenGui")
Screen.Name           = "GNS_HubUI_v3"
...
-- drag with click-vs-drag separation
local function makeDraggable(handle, target)
    local dragging, didDrag = false, false
    local startMouse, startPos = nil, nil
    local THRESH = 6
...
-- orbit dots (parented to Screen so they escape the clipped circle)
local orbitDots = {}
...
for i = 1, 10 do
    ...
    orbitDots[i] = {
        dot   = dot,
        angle = math.rad((i-1) * 36),
        speed = 0.7 + math.random() * 0.9,
        r     = IC/2 + 14 + math.random() * 6,
        sz    = sz,
    }
end

-- GNS text (color cycles)
...
mk("TextLabel", {
    Size=UDim2.new(0,16,0,16), Position=UDim2.new(0.5,-8,0,2),
...
local function tlight(x, col)
    local f = mk("Frame",{
        Size=UDim2.new(0,13,0,13), Position=UDim2.new(0,x,0.5,-6),
...
mk("TextLabel",{
    ...
    BackgroundTransparency=1, Text="⚡ GNS HUB v3",
    TextColor3=C.accentHi, TextSize=13,
    Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
},TitleBar)

-- version badge
local vBadge = mk("Frame",{
    Size=UDim2.new(0,46,0,20), Position=UDim2.new(0,190,0.5,-10),
    BackgroundColor3=C.accentDim, BorderSizePixel=0,
},TitleBar)
corner(6,vBadge)
mk("TextLabel",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="v3.0",TextColor3=C.accentHi,TextSize=10,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Center},vBadge)
...
corner(999,liveDot)
mk("TextLabel",{
    Size=UDim2.new(0,44,1,0), Position=UDim2.new(1,-50,0,0),
    BackgroundTransparency=1, Text="LIVE",
...
mk("TextLabel",{Size=UDim2.new(1,0,0,16),Position=UDim2.new(0,0,0,30),BackgroundTransparency=1,Text="Steal an Egg • v3.0",TextColor3=C.subtext,TextSize=10,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left},HubBlock)
...
local function sideNav(label, icon, tabId, order)
    ...
    local activeBg = mk("Frame",{
        Size=UDim2.new(1,0,1,0), BackgroundColor3=C.card,
        BackgroundTransparency=1, BorderSizePixel=0,
    },btn)
    corner(10,activeBg)
    local bar = mk("Frame",{
    ...
    local lbl = mk("TextLabel",{
        ...
        TextColor3=C.subtext, TextSize=12,
    ...
    btn.MouseEnter:Connect(function()
        if not bar.Visible then
            TweenService:Create(activeBg, TweenInfo.new(0.1), {BackgroundTransparency=0.7}):Play()
        end
    end)
    btn.MouseLeave:Connect(function()
        if not bar.Visible then
            TweenService:Create(activeBg, TweenInfo.new(0.1), {BackgroundTransparency=1}):Play()
        end
    end)
...
local function makeSection(parent, title, titleCol, order)
    ...
    local accBar = mk("Frame",{
        Size=UDim2.new(0,3,0,22), Position=UDim2.new(0,0,0.5,-11),
        BackgroundColor3=titleCol or C.accent, BorderSizePixel=0,
    },hdr)
    pad(18,16,0,0,hdr)
    mk("TextLabel",{
        Size=UDim2.new(1,0,1,0), BackgroundTransparency=1,
        Text=title, TextColor3=titleCol or C.text,
...
local function divLine(parent, lo)
    mk("Frame",{
        Size=UDim2.new(1,-32,0,1), Position=UDim2.new(0,16,0,0),
        BackgroundColor3=C.border, BorderSizePixel=0, LayoutOrder=lo or 99,
    },parent)
end
...
divLine(asLBody,6)
local togRideMount = makeRowToggle(asLBody,"Ride Mount After Steal",true,7,C.purple)
divLine(asLBody,8)
...
for i, r in ipairs(RARITY_DATA) do
    ...
    mk("TextLabel",{
        ...
        BackgroundTransparency=1, Text=r.emoji.." "..r.name,
...
local function buildEggRow(data, idx, parent)
    ...
    -- rarity emoji
    mk("TextLabel",{
        Size=UDim2.new(0,26,0,26), Position=UDim2.new(0,12,0,8),
        BackgroundTransparency=1, Text=rd.emoji,
    ...
    -- egg name
    mk("TextLabel",{
        Size=UDim2.new(1,-140,0,18), Position=UDim2.new(0,42,0,5),
    ...
    -- pet name (bold, main info)
    mk("TextLabel",{
        Size=UDim2.new(1,-140,0,18), Position=UDim2.new(0,42,0,22),
    ...
    -- rarity + dist + money
    local moneyStr = data.money ~= "" and ("  💰"..data.money) or ""
    mk("TextLabel",{
        Size=UDim2.new(1,-140,0,15), Position=UDim2.new(0,42,0,43),
        BackgroundTransparency=1,
        Text=rd.emoji.." "..data.rarity.."  📍"..data.dist.."m"..moneyStr,
...
local function buildPredictRow(data,idx,parent)
    local rd  = getRD(data.rarity)
    local row = mk("Frame",{Size=UDim2.new(1,0,0,64),BackgroundColor3=C.panel,BorderSizePixel=0,LayoutOrder=idx},parent)
    corner(10,row)
    uistroke(rd.color, 0.8, row)
    local stripe=mk("Frame",{Size=UDim2.new(0,4,1,-8),Position=UDim2.new(0,4,0,4),BackgroundColor3=rd.color,BorderSizePixel=0},row)
    corner(4,stripe)
    mk("TextLabel",{Size=UDim2.new(1,-20,0,16),Position=UDim2.new(0,14,0,4),BackgroundTransparency=1,Text=data.name,TextColor3=C.subtext,TextSize=9,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},row)
    mk("TextLabel",{Size=UDim2.new(1,-20,0,18),Position=UDim2.new(0,14,0,20),BackgroundTransparency=1,Text="🐾 "..data.pet,TextColor3=C.text,TextSize=12,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},row)
    local monStr = data.money ~= "" and ("  💰"..data.money) or ""
    mk("TextLabel",{Size=UDim2.new(1,-20,0,14),Position=UDim2.new(0,14,0,40),BackgroundTransparency=1,Text=rd.emoji.." "..data.rarity.."  📍"..data.dist.."m"..monStr,TextColor3=rd.color,TextSize=9,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left},row)
end

-- ─────────────────────────────────────────────────────────────
-- TAB: EGG ESP (persistent billboards — NO REBUILD LAG)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- ESP ENGINE — persistent BillboardGuis, updated via Heartbeat
-- Key insight: we keep a table of {part -> billboardData}
-- and only create/destroy when the egg set changes.
-- Text updates happen every ~0.5s via a counter, not via Destroy/Create.
-- ─────────────────────────────────────────────────────────────
local espBoards = {}   -- part -> {bill, nameLbl, petLbl, rarLbl, monLbl}

local function clearESP()
    for _, bd in pairs(espBoards) do
...
local function buildESPBill(data)
    ...
    if espBoards[part] then return end  -- already exists

    local rd = getRD(data.rarity)
    local bill = Instance.new("BillboardGui")
    bill.Size           = UDim2.new(0, 145, 0, 62)
    ...
    corner(8,bg)
    uistroke(rd.color, 1.8, bg)

    -- left color stripe
    local stripe = mk("Frame",{
        Size=UDim2.new(0,3,1,-6), Position=UDim2.new(0,2,0,3),
        BackgroundColor3=rd.color, BorderSizePixel=0,
    },bg)
    corner(2,stripe)

    local nameLbl = mk("TextLabel",{
        Size=UDim2.new(1,-10,0,13), Position=UDim2.new(0,9,0,2),
        BackgroundTransparency=1, Text=data.name,
        TextColor3=C.subtext, TextSize=8,
        Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
        TextTruncate=Enum.TextTruncate.AtEnd,
    ...
    local petLbl = mk("TextLabel",{
        Size=UDim2.new(1,-10,0,16), Position=UDim2.new(0,9,0,15),
        BackgroundTransparency=1, Text="🐾 "..data.pet,
        TextColor3=Color3.new(1,1,1), TextSize=11,
        Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
        TextTruncate=Enum.TextTruncate.AtEnd,
    ...
    local rarLbl = mk("TextLabel",{
        Size=UDim2.new(1,-10,0,13), Position=UDim2.new(0,9,0,31),
        BackgroundTransparency=1, Text=rd.emoji.." "..data.rarity,
        TextColor3=rd.color, TextSize=9,
        Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
    },bg)

    local moneyStr = data.money ~= "" and ("💰"..data.money) or ""
    local monLbl = mk("TextLabel",{
        Size=UDim2.new(1,-10,0,12), Position=UDim2.new(0,9,0,46),
        BackgroundTransparency=1, Text="",
        TextColor3=C.gold, TextSize=8,
        Font=Enum.Font.Gotham, TextXAlignment=Enum.TextXAlignment.Left,
    },bg)

    espBoards[part] = {
        bill=bill, bg=bg, stripe=stripe,
        nameLbl=nameLbl, petLbl=petLbl, rarLbl=rarLbl, monLbl=monLbl,
        lastDist="", lastMoney=moneyStr,
    }
end
...
local function refreshESPText(data)
    ...
    local moneyStr = data.money ~= "" and ("💰"..data.money) or ""
    -- only update text if changed (avoid unnecessary renders)
    if bd.lastDist ~= distStr then
        bd.rarLbl.Text = rd.emoji.." "..data.rarity.."  📍"..distStr
        bd.lastDist    = distStr
    end
    if bd.lastMoney ~= moneyStr then
        bd.monLbl.Text  = moneyStr
        bd.lastMoney    = moneyStr
    end
end
...
mk("TextLabel",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,Text="Auto-walks the treadmill to earn eggs faster.",TextColor3=C.subtext,TextSize=11,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true},tmDescF)
divLine(tmLBody,2)
local togTreadmill = makeRowToggle(tmLBody,"Auto Treadmill",false,3,C.green)
divLine(tmLBody,4)

-- Treadmill auto-walk logic
local treadConn = nil
    ...
    if v then
        treadConn = RunService.Heartbeat:Connect(function()
            pcall(function()
                local root = getRoot()
                if not root then return end
                -- look for treadmill zone and spam touch
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("BasePart") then
                        local nm = obj.Name:lower()
...
local tmStatus=mk("TextLabel",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,Text="Status: Idle",TextColor3=C.dim,TextSize=11,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=1},tmRBody)
togTreadmill.onChange(function(v)
...
divLine(rfLBody,2)
local togRiftFarm = makeRowToggle(rfLBody,"Farm Missing Rift Pets",false,3,C.cyan)
divLine(rfLBody,4)
local togRiftAuto = makeRowToggle(rfLBody,"Auto Submit & Claim",false,5,C.purple)
divLine(rfLBody,6)
...
local liveStateF=mk("Frame",{Size=UDim2.new(1,0,0,0),BackgroundTransparency=1,AutomaticSize=Enum.AutomaticSize.Y,LayoutOrder=1},lsBody)
pad(16,16,8,8,liveStateF)
mk("TextLabel",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,Text="Dragon event: Inactive\nPhase: Inactive\nZone: Waiting",TextColor3=C.dim,TextSize=10,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true},liveStateF)

-- ─────────────────────────────────────────────────────────────
-- SCAN LOOP — runs every 1.0s (not 0.5s — reduces scan cost)
-- Only rebuilds rows when tab is visible
-- ESP boards are NOT destroyed each cycle — only text updated
-- ─────────────────────────────────────────────────────────────
local scanTick = 0
task.spawn(function()
    while true do
        ...
        -- update distances (cheap)
        local root = getRoot()
        if root then
            for _, data in ipairs(eggs) do
                local ok2, d = pcall(function()
                    return math.floor((root.Position - data.pos).Magnitude)
                end)
                if ok2 then data.dist = d end
            end
        end

        -- AUTO STEAL TAB egg list rebuild
        ...
        -- ESP — build boards for new eggs, remove stale ones
        if State.espEnabled then
            -- collect current parts set
            local currentParts = {}
            for _, data in ipairs(eggs) do
                if data.part and data.part.Parent then
                    currentParts[data.part] = data
                end
            end
            -- remove boards for eggs no longer present
        ...
        if #eggs > 0 then
            local e = eggs[1]
            local rd = getRD(e.rarity)
            lrStatus.Text       = e.name.."\n🐾 "..e.pet.."\n"..rd.emoji.." "..e.rarity.."  📍"..e.dist.."m"..(e.money~="" and "\n💰 "..e.money or "")
        ...
        for _, data in ipairs(eggs) do
            ...
            if r=="Secret" or r=="Eternal" or r=="Mythic" or r=="Cosmic" or r=="Divine" then
                table.insert(dEggs, getRD(r).emoji.." "..data.pet.." ["..r.."]")
            end
        end
...
-- ─────────────────────────────────────────────────────────────
-- AUTO STEAL LOOP — smooth, no double-fire
-- Runs every 0.2s, but only acts when not already stealing
-- ─────────────────────────────────────────────────────────────
...
print("⚡ GNS HUB v3 LOADED — Click the GNS circle to open! (Nono Edition)")
