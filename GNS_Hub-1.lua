...
local function makeDraggable(handle, target)
    ...
    local moved   = false
    local ds, sp  = nil, nil
    local THRESH  = 6 -- pixels moved before it counts as a drag
    ...
    UserInputService.InputChanged:Connect(function(inp)
        if drag and (
            ...
            if not moved and delta.Magnitude >= THRESH then
                moved = true
            end
            if moved then
    ...
    -- return whether last press was a real drag (so click handler can ignore it)
...
-- // ─────────────────────────────────────────────
-- // ─────────────────────────────────────────────
-- // GNS LOGO ICON — 56px, logo visible, drag+click both work
-- // ─────────────────────────────────────────────
local ICON_SIZE = 56
local ICON_HALF = ICON_SIZE / 2
local ORBIT_R   = ICON_HALF + 10  -- orbit radius just outside circle

local IconWrap = mk("Frame", {
    Size             = UDim2.new(0, ICON_SIZE, 0, ICON_SIZE),
    Position         = UDim2.new(0, 14, 0.5, -ICON_HALF),
    BackgroundColor3 = Color3.fromRGB(10, 13, 30),
    BorderSizePixel  = 0,
...
-- Border ring
local borderRing = mk("UIStroke", {
    Color     = C.accent,
    Thickness = 2,
}, IconWrap)

-- Glow pulse ring 1
local GlowRing1 = mk("Frame", {
...
corner(999, GlowRing1)
local gs1 = mk("UIStroke", {Color=C.accent, Thickness=7}, GlowRing1)
gs1.Transparency = 0.6
...
local gs2 = mk("UIStroke", {Color=C.cyan, Thickness=3}, GlowRing2)
gs2.Transparency = 0.78
TweenService:Create(gs2, TweenInfo.new(2.0, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {Transparency=0.96}):Play()

-- Orbiting dots — parented to Screen so they orbit OUTSIDE the clipped circle
...
for i = 1, ORBIT_COUNT do
    ...
    local dot = mk("Frame", {
        ...
        BorderSizePixel  = 0,
        ZIndex           = 20,
        Position         = UDim2.new(0, -99, 0, -99), -- offscreen until heartbeat
    }, Screen)
    corner(999, dot)
    local angle = math.rad((i-1) * (360 / ORBIT_COUNT))
    orbitParticles[i] = {dot=dot, angle=angle, speed=1.0 + math.random()*0.5}
end

local orbitTime = 0
RunService.Heartbeat:Connect(function(dt)
    orbitTime = orbitTime + dt
    -- get absolute center of IconWrap each frame (handles dragging)
    local absPos = IconWrap.AbsolutePosition
    local cx = absPos.X + ICON_HALF
    local cy = absPos.Y + ICON_HALF
    for _, p in ipairs(orbitParticles) do
        local a  = p.angle + orbitTime * p.speed
        local px = cx + math.cos(a) * ORBIT_R - 2
        local py = cy + math.sin(a) * ORBIT_R - 2
        p.dot.Position = UDim2.new(0, px, 0, py)
        p.dot.BackgroundTransparency = 0.1 + 0.6 * math.abs(math.sin(orbitTime * 1.3 + p.angle))
    end
end)

-- GNS label — high ZIndex so it shows over everything
...
-- Invisible button on top — handles BOTH drag and click separately
...
local wasDragged = makeDraggable(IconBtn, IconWrap)
...
IconBtn.MouseButton1Click:Connect(function()
    -- if user was dragging, don't toggle the window
