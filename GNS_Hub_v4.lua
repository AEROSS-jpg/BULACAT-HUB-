-- // ============================================================
-- // ⚡ GNS HUB v4 — FULL FIX (Nono Edition)
-- // Game: Steal An Egg
-- // Executor: KRNL / Synapse X / Fluxus / Delta
-- // FIXES v4:
-- //   ✅ Auto steal INSTANT — no backing, teleport to center line first
-- //   ✅ Steal sequence: center → egg → interact → back to line FAST
-- //   ✅ Real pet name from egg internals (StringValue, BillboardGui, TextLabel)
-- //   ✅ Real egg name shown (actual object name cleaned)
-- //   ✅ Real Money/s from NumberValue/IntValue/Attributes
-- //   ✅ Filter by rarity works properly
-- //   ✅ Auto Treadmill wired and working
-- //   ✅ God mode Heartbeat-enforced (truly unkillable during steal)
-- //   ✅ Monster ride after steal (Miranda style)
...
-- ─────────────────────────────────────────────────────────────
-- PET PREDICTION — 100% from real egg data
-- ─────────────────────────────────────────────────────────────
...
local function predictPet(obj)
    -- Pass 1: StringValue with pet-key name
    for _,v in ipairs(obj:GetDescendants()) do
    ...
    -- Pass 2: BillboardGui TextLabels (name tags floating above egg)
    for _,v in ipairs(obj:GetDescendants()) do
    ...
    -- Pass 3: TextLabel or TextButton
    for _,v in ipairs(obj:GetDescendants()) do
    ...
    -- Pass 5: sibling StringValues in parent model
    if obj.Parent then
        for _,sib in ipairs(obj.Parent:GetChildren()) do
    ...
    -- Pass 6: derive from egg name (clean it up)
...
-- ─────────────────────────────────────────────────────────────
-- REAL EGG NAME — cleaned actual object name
-- ─────────────────────────────────────────────────────────────
local function getEggName(obj)
    local n = obj.Name
    -- try parent model name first (more descriptive)
    if obj.Parent and obj.Parent:IsA("Model") then
        n = obj.Parent.Name
    end
    -- clean underscores/camelCase spacing
...
-- Finds the center/start position of the steal line in the map
-- so player can snap there before heading to egg (avoids map edge)
-- ─────────────────────────────────────────────────────────────
...
local function findLineCenter()
    ...
    -- search for "line", "start", "spawn", "center", "queue", "waiting" parts
    ...
    -- fallback: use player's own current position as "home"
    return root.Position
end

-- ─────────────────────────────────────────────────────────────
-- EGG SCANNER — full workspace recurse
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- FAST TELEPORT (instant CFrame snap — no fly engine needed for steal)
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- FLY ENGINE (used only for Hold Longest)
-- ─────────────────────────────────────────────────────────────
local flyConn = nil
local flyBV   = nil
local flyBG   = nil
...
-- ─────────────────────────────────────────────────────────────
-- STEAL EGG v4 — INSTANT, NO BACKING
-- Sequence:
--   1. God mode ON
--   2. Snap to center of line (stay in bounds)
--   3. Snap directly ON the egg (CFrame teleport — instant, no fly)
--   4. Fire all interactions
--   5. Snap back to line center (instant return — no backing lag)
--   6. Ride monster / speed boost
--   7. God mode OFF
-- ─────────────────────────────────────────────────────────────
local homePos = nil  -- cached line center
...
local function stealEgg(data, onDone)
    ...
    -- Step 1: snap to line center first
    if homePos then
        snapTo(homePos)
        task.wait(0.05)
    end

    -- Step 2: snap directly onto egg
    local eggPos = data.pos
    snapTo(Vector3.new(eggPos.X, eggPos.Y + 3, eggPos.Z))
    task.wait(0.06)

    -- Step 3: fire all triggers
    local r2 = getRoot()
    if r2 then
        fireAllInteractions(data.obj, r2)
        -- also try parent model
        if data.obj:IsA("BasePart") and data.obj.Parent and data.obj.Parent:IsA("Model") then
            fireAllInteractions(data.obj.Parent, r2)
        end
    end

    task.wait(0.12)

    -- Step 4: instant snap back to line center
    if homePos then
        snapTo(homePos)
    end
    ...
    -- Step 5: ride or speed boost
    if togRideMount and togRideMount.getOn() then
        rideBack()
    end
...
-- ─────────────────────────────────────────────────────────────
pcall(function()
    for _,name in ipairs({"GNS_HubUI_v4","GNS_HubUI_v3","GNS_HubUI"}) do
...
mk("TextLabel",{
    ...
    BackgroundTransparency=1, Text="⚡ GNS HUB v4",
...
corner(6,vBadge)
mk("TextLabel",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="v4.0",TextColor3=C.accentHi,TextSize=10,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Center},vBadge)
...
mk("TextLabel",{Size=UDim2.new(1,0,0,16),Position=UDim2.new(0,0,0,30),BackgroundTransparency=1,Text="Steal an Egg • v4.0",TextColor3=C.subtext,TextSize=10,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left},HubBlock)
...
divLine(asLBody,6)
togRideMount       = makeRowToggle(asLBody,"Ride Mount After Steal",true,7,C.purple)
divLine(asLBody,8)
...
local function buildEggRow(data,idx,parent)
    ...
    corner(4,stripe)
    mk("TextLabel",{Size=UDim2.new(0,26,0,26),Position=UDim2.new(0,12,0,8),BackgroundTransparency=1,Text=rd.emoji,TextSize=16,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Center},row)

    -- egg name (real, from object)
    mk("TextLabel",{
    ...
    -- pet name (bold, real from internals)
    mk("TextLabel",{
...
local function buildPredictRow(data,idx,parent)
    ...
    local stripe=mk("Frame",{Size=UDim2.new(0,4,1,-8),Position=UDim2.new(0,4,0,4),BackgroundColor3=rd.color,BorderSizePixel=0},row)
    corner(4,stripe)
    mk("TextLabel",{Size=UDim2.new(1,-20,0,14),Position=UDim2.new(0,14,0,3),BackgroundTransparency=1,Text="🥚 "..data.name,TextColor3=C.subtext,TextSize=9,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},row)
    mk("TextLabel",{Size=UDim2.new(1,-20,0,18),Position=UDim2.new(0,14,0,18),BackgroundTransparency=1,Text="🐾 "..data.pet,TextColor3=C.text,TextSize=12,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},row)
    local monStr = data.money ~= "" and ("  💰"..data.money) or ""
    mk("TextLabel",{Size=UDim2.new(1,-20,0,14),Position=UDim2.new(0,14,0,38),BackgroundTransparency=1,Text=rd.emoji.." "..data.rarity.."  📍"..data.dist.."m"..monStr,TextColor3=rd.color,TextSize=9,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left},row)
...
local function buildESPBill(data)
    ...
    corner(2,stripe)
    local nameLbl=mk("TextLabel",{Size=UDim2.new(1,-10,0,13),Position=UDim2.new(0,9,0,2),BackgroundTransparency=1,Text="🥚 "..data.name,TextColor3=C.subtext,TextSize=8,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},bg)
    local petLbl=mk("TextLabel",{Size=UDim2.new(1,-10,0,16),Position=UDim2.new(0,9,0,15),BackgroundTransparency=1,Text="🐾 "..data.pet,TextColor3=Color3.new(1,1,1),TextSize=11,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},bg)
    local rarLbl=mk("TextLabel",{Size=UDim2.new(1,-10,0,13),Position=UDim2.new(0,9,0,31),BackgroundTransparency=1,Text=rd.emoji.." "..data.rarity,TextColor3=rd.color,TextSize=9,Font=Enum.Font.GothamBold,TextXAlignment=Enum.TextXAlignment.Left},bg)
...
-- Treadmill logic — Heartbeat spam touch on all treadmill parts
local treadConn  = nil
local treadParts = {}  -- cached treadmill parts
...
togTreadmill.onChange(function(v)
    if v then
        cacheTreadmillParts()
        if #treadParts == 0 then
            -- retry scan after 2s to give game time to load
            task.delay(2, cacheTreadmillParts)
        end
            ...
            pcall(function()
                ...
                for _,obj in ipairs(treadParts) do
                    if obj and obj.Parent then
                        ...
                        -- also try ClickDetector and ProximityPrompt
                        local cl = obj:FindFirstChildOfClass("ClickDetector")
                ...
                -- also spam any RemoteEvent named treadmill/walk/run
...
-- ─────────────────────────────────────────────────────────────
-- SCAN LOOP — 1s interval, updates all tabs
-- ─────────────────────────────────────────────────────────────
task.spawn(function()
    while true do
        task.wait(1.0)
...
-- ─────────────────────────────────────────────────────────────
-- AUTO STEAL LOOP — fast, no double fire, no backing
-- Runs every 0.15s — only acts when not already stealing
-- ─────────────────────────────────────────────────────────────
...
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(1)
    State.stealing = false
    homePos        = nil  -- reset line center cache on respawn
    lineCenterCache= nil
    pcall(stopFly)
    pcall(function() if rideWeld then rideWeld:Destroy() rideWeld=nil end end)
    if holdActive then
...
print("⚡ GNS HUB v4 LOADED — Click the GNS circle to open!")
