-- // ============================================
-- // 🐱 BULACAT HUB v6 — CONFIDENCE EDITION
-- // Score-Based Scanner | Persistent Fly Loop
-- // Smooth Steal | No Jitter | No Churn
-- // ============================================
...
local CFG = {
    ...
    HOLD_HEIGHT    = 350,
    PARTICLE_COUNT = 22,
    MIN_CONFIDENCE = 3,  -- egg candidates need score >= this
}
...
local WEAK_WORDS   = {"pet","animal","creature","prize","spin","drop","roll","summon"}

local HATCH_ACTIONS = {"hatch","open","claim","collect","unlock","buy egg","spin","summon","roll"}
...
local BLACKLIST_NAMES = {
    ...
    ["portal"]=true, ["shrine"]=true, ["statue"]=true, ["decal"]=true,
    ["handle"]=true, ["ladder"]=true, ["chair"]=true, ["seat"]=true, ["vehicle"]=true,
}
...
-- Score an object as an egg candidate (0 = definitely not, higher = more likely)
local function scoreEgg(obj)
    local score = 0
    local reasons = {}

    -- Reject anything inside a Humanoid model
    local check = obj
    local depth = 0
    while check and depth < 5 do
        if check:IsA("Model") and check:FindFirstChildOfClass("Humanoid") then
            return 0, {"rejected: humanoid"}
        end
        check = check.Parent; depth = depth + 1
    end

    -- Reject service containers
    local top = obj
    while top and top.Parent and top.Parent ~= workspace and top.Parent ~= game do
        top = top.Parent
    end
    if top and top ~= workspace then return 0, {"rejected: not in workspace"} end

    -- Name match
    local nm = obj.Name:lower()
    for _,b in pairs(BLACKLIST_NAMES) do
        if nm == b then return 0, {"blacklisted: "..b} end
    end
    if containsAny(nm, STRONG_WORDS) then score = score + 4; table.insert(reasons, "name:strong") end
    elseif containsAny(nm, WEAK_WORDS) then score = score + 1; table.insert(reasons, "name:weak") end

    -- Parent name match
    if obj.Parent and obj.Parent:IsA("Model") then
        local pn = obj.Parent.Name:lower()
        if containsAny(pn, STRONG_WORDS) then score = score + 2; table.insert(reasons, "parent:strong") end
        if containsAny(pn, WEAK_WORDS) then score = score + 1; table.insert(reasons, "parent:weak") end
    end

    -- Attribute match
    for _,key in ipairs({"Egg","IsEgg","Type","Category","ObjectType","ItemType","Kind","Class","Tag"}) do
        local ok,val = pcall(function() return obj:GetAttribute(key) end)
        if ok and val then
            local vs = tostring(val):lower()
            if containsAny(vs, STRONG_WORDS) then score = score + 3; table.insert(reasons, "attr:strong") end
            if containsAny(vs, WEAK_WORDS)   then score = score + 1; table.insert(reasons, "attr:weak")   end
        end
    end

    -- CollectionService tag match
    for _,tag in ipairs(CollectionService:GetTags(obj)) do
        local tl = tag:lower()
        if containsAny(tl, STRONG_WORDS) then score = score + 3; table.insert(reasons, "tag:strong") end
        if containsAny(tl, WEAK_WORDS)   then score = score + 1; table.insert(reasons, "tag:weak")   end
    end

    -- Direct ProximityPrompt with strong action
    local prompt = obj:FindFirstChildOfClass("ProximityPrompt")
    if prompt then
        local act = (prompt.ActionText or ""):lower()
        local otx = (prompt.ObjectText or ""):lower()
        for _,hw in ipairs(HATCH_ACTIONS) do
            if act:find(hw, 1, true) then score = score + 3; table.insert(reasons, "prompt:action") break end
        end
        if containsAny(otx, STRONG_WORDS) then score = score + 2; table.insert(reasons, "prompt:strong") end
        if containsAny(otx, WEAK_WORDS)   then score = score + 1; table.insert(reasons, "prompt:weak")   end
    end

    -- Nearby StringValues / TextLabels (only immediate children for speed)
    for _,v in ipairs(obj:GetChildren()) do
        if v:IsA("StringValue") then
            local val = (v.Value or ""):lower()
            if containsAny(val, STRONG_WORDS) then score = score + 2; table.insert(reasons, "child:string") break end
        elseif v:IsA("TextLabel") or v:IsA("TextButton") then
            local val = (v.Text or ""):lower()
            if containsAny(val, STRONG_WORDS) then score = score + 1; table.insert(reasons, "child:label") break end
        end
    end
...
local function scanEggs()
    ...
    local function isScannable(obj)
        ...
        return cls == "Part" or cls == "MeshPart" or cls == "UnionOperation"
            or cls == "Model" or cls == "SpawnLocation" or cls == "TrussPart"
    end
        ...
        if score >= CFG.MIN_CONFIDENCE then
            ...
            if center then
                ...
                table.insert(found, {
                    ...
                    dist=dist, region=region, score=score,
    ...
    -- scan characters too (in case eggs on players)
                    ...
                    if isScannable(obj) then
                        ...
                        if score >= CFG.MIN_CONFIDENCE then
                            ...
                            if center then
                                ...
                                local pet    = extractPetName(obj) or (region.." Egg")
                                local dist   = math.floor((rootPos - center).Magnitude)
                                table.insert(found, {egg=obj,center=center,rarity=rar,pet=pet,dist=dist,region=region,score=score,eta=dist/CFG.FLY_SPEED})
                            end
                        end
                    end
                end
    ...
    -- sort: highest score first, then rarest, then nearest
...
-- // ─────────────────────────────────────────────
-- // PERSISTENT FLY LOOP (no churn, one connection ever)
-- // ─────────────────────────────────────────────
local flyState = {
    active = false,
    target = nil,
...
local function requestFly(targetPos, speed, onArrive)
    ...
    flyState.arrived = false
    pcall(function() getHuman().PlatformStand = true end)
end
...
-- ONE Heartbeat connection for flying, runs forever
RunService.Heartbeat:Connect(function()
    ...
    if dist < 30 then s = math.clamp(dist * 15, 40, flyState.speed) end
...
-- // ─────────────────────────────────────────────
-- // STEAL — 5 studs above, instant interact
-- // ─────────────────────────────────────────────
...
-- // ─────────────────────────────────────────────
pcall(function()
    for _,n in ipairs({"BulacatHubV4","BulacatHubV5","BulacatHubV51","BulacatHubV52","BulacatHubV53","BulacatHubV6"}) do
        local o = CoreGui:FindFirstChild(n); if o then o:Destroy() end
    end
end)

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name            = "BulacatHubV6"
...
mk("TextLabel",{Size=UDim2.new(1,0,0,12),Position=UDim2.new(0,0,1,2),BackgroundTransparency=1,Text="BH v6",TextSize=8,Font=Enum.Font.GothamBold,TextColor3=C.accentHi,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=43},LogoOuter)
...
mk("TextLabel",{Size=UDim2.new(0,200,0,12),Position=UDim2.new(0,44,0,24),BackgroundTransparency=1,Text="Egg Stealer v6 • Confidence",TextColor3=C.subtext,TextSize=9,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=12},Header)
...
local function buildRow(data, idx)
    ...
    mk("TextLabel",{Size=UDim2.new(1,-108,0,10),Position=UDim2.new(0,34,0,19),BackgroundTransparency=1,Text=data.rarity.." • "..data.region.." • score "..tostring(data.score),TextColor3=rd.color,TextSize=8,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=14},row)
...
local function runAutoStealTick()
    ...
    if not autoOn then stealOnceBusy = false; stealOnceDone = false; return end
    if State.stealing then return end
    if onceOn then
        ...
        if ok and eggs and #eggs > 0 then
            ...
            for _,data in ipairs(eggs) do
                if State.selectedRar[data.rarity] and data.egg and data.egg.Parent then target = data; break end
            end
            if target then
                State.stealing = true
...
print("🐱 BULACAT HUB v6 CONFIDENCE — Click the logo!")