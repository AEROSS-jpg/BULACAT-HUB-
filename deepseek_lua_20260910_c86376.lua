-- // ============================================
-- // 🪓 AXE'S HUB v1.2 — FULL BUILD (EXECUTION FIXED)
-- // Pixel-Axe Logo | Gold Theme | Score Scanner
-- // Smooth Steal | 5-Stud Hover | Auto + Once
-- // ============================================
...
-- // ─────────────────────────────────────────────
local RARITY = {
    {name="Divine",   emoji="D", color=Color3.fromRGB(255,215,0),   rank=1},
    {name="Eternal",  emoji="E", color=Color3.fromRGB(180,0,255),   rank=2},
    {name="Secret",   emoji="S", color=Color3.fromRGB(150,150,150), rank=3},
    {name="Cosmic",   emoji="C", color=Color3.fromRGB(160,0,255),   rank=4},
    {name="Legendary",emoji="L", color=Color3.fromRGB(255,140,0),   rank=5},
    {name="Epic",     emoji="P", color=Color3.fromRGB(100,50,255),  rank=6},
    {name="Rare",     emoji="R", color=Color3.fromRGB(0,120,255),   rank=7},
    {name="Uncommon", emoji="U", color=Color3.fromRGB(0,200,80),    rank=8},
    {name="Common",   emoji="X", color=Color3.fromRGB(180,180,180), rank=9},
}
local RARITY_MAP = {}
...
-- // UTIL
-- // ─────────────────────────────────────────────
local STRONG_WORDS = {"egg","crate","chest","hatch","capsule","present","gift","lootbox","reward"}
...
local function scoreEgg(obj)
    ...
    if containsAny(nm, STRONG_WORDS) then score = score + 4
    elseif containsAny(nm, WEAK_WORDS) then score = score + 1 end
    ...
    if prompt then
        ...
        for _,hw in ipairs(HATCH_ACTIONS) do
            if act:find(hw, 1, true) then score = score + 3; break end
        end
...
local function extractPetName(obj)
    ...
    for _,k in ipairs(attrList) do
        ...
        if ok and val ~= nil and type(val) == "string" then
            ...
            if c and not string.lower(c):find("egg") and not string.lower(c):find("rarity") then return c end
        end
    end
    ...
    for _,v in ipairs(desc) do
        if v:IsA("ProximityPrompt") then
            ...
            if c and not string.lower(c):find("hatch") and not string.lower(c):find("open") and not string.lower(c):find("egg") and not string.lower(c):find("click") then return c end
        end
    end
    for _,v in ipairs(desc) do
        if v:IsA("TextLabel") or v:IsA("TextButton") then
            ...
            if c then
                ...
                if not cl:find("egg") and not cl:find("rarity") and not cl:find("hatch") and not cl:find("click") and not cl:find("open") and not cl:find("buy") then return c end
            end
        end
    end
...
local function extractRarity(obj)
    ...
    for _,v in ipairs(desc) do
        if v:IsA("StringValue") then
            local vn = string.lower(v.Name)
            if vn:find("rar") or vn:find("tier") then table.insert(sources, string.lower(v.Value or "")) end
        end
        if v:IsA("TextLabel") or v:IsA("TextButton") then
    ...
    for _,v in ipairs(desc) do
        if v:IsA("StringValue") then table.insert(sources, string.lower(v.Value or "")) end
    end
...
local function regionLabel(obj)
    ...
    while p ~= nil and depth < 8 do
        if p ~= workspace and p.Parent == workspace and p.Name ~= "Camera" then
            local nm = p.Name
            if #nm >= 2 and #nm <= 30 and string.lower(nm) ~= "workspace" then return nm end
        end
        p = p.Parent
        depth = depth + 1
    end
...
local function isScannable(obj)
    ...
    or cls == "Model" or cls == "SpawnLocation" or cls == "TrussPart" then return true end
    return false
end
...
-- // FLY
-- // ─────────────────────────────────────────────
local flyState = { active=false, target=nil, speed=CFG.FLY_SPEED, arriveDist=CFG.ARRIVE_DIST, onArrive=nil }
local flyBV = nil
local flyBG = nil
...
local function startHoldLongest(statusFn, dotFn)
    ...
    holdConn = RunService.Heartbeat:Connect(function()
        ...
        if reached then
            ...
            if diff.Magnitude > 2 then holdBV.Velocity = diff.Unit * 60
            else holdBV.Velocity = Vector3.new(0, 0, 0) end
            local all = workspace:GetDescendants()
            for _,obj in ipairs(all) do
...
-- // ─────────────────────────────────────────────
pcall(function()
    for _,n in ipairs({"BulacatHubV4","BulacatHubV5","BulacatHubV51","BulacatHubV52","BulacatHubV53","BulacatHubV6","BulacatHubV61","AxesHubV1","AxesHubV12"}) do
...
local PARTICLE_COLORS = {C.goldHi, C.gold, C.orange, C.blade, C.goldDim}
local particles = {}
...
local FILTER_DATA = {
    {name="Divine",   row=FR1},
    {name="Eternal",  row=FR1},
    {name="Secret",   row=FR1},
    {name="Cosmic",   row=FR2},
    {name="Legendary",row=FR2},
    {name="Epic",     row=FR2},
}
...
local function buildRow(data, idx)
    ...
    stBtn.MouseButton1Click:Connect(function()
        ...
        State.stealing = true
        setStatus("Flying to "..data.pet, C.gold)
...
local function runAutoStealTick()
    ...
    if onceOn then
        ...
        if ok and eggs and #eggs > 0 then
            ...
            if target then
                State.stealing = true
                setStatus("[ONCE] Flying to "..target.pet, C.gold)
                stealEgg(target.egg, target.center, function(success)
                    ...
                    setAutoOn(false)
                    setOnceOn(false)
                    setStatus(success and ("[ONCE] Got "..target.pet) or "[ONCE] failed", success and C.green or C.red)
                end)
            else
                stealOnceBusy = false
    ...
    if target then
        State.stealing = true
        setStatus("Auto flying to "..target.pet.." ["..target.rarity.."]", C.gold)
        stealEgg(target.egg, target.center, function(success)
            State.stealing = false
...
print("[AXE'S HUB v1.2] Loaded. Click the axe logo!")