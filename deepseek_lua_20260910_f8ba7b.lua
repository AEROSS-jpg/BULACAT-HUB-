-- // ============================================
-- // 🐱 BULACAT HUB v5.2 — Smooth Steal + 5-Stud Hover
-- // State-Machine Flight | One-Shot Latch | Trap-Safe
-- // ============================================
...
local CFG = {
    ...
    FLY_SPEED      = 750,
    APPROACH_DIST  = 2.5,
    HOVER_ABOVE    = 5,         -- 5 studs above egg center
    HOLD_HEIGHT    = 350,
    PARTICLE_COUNT = 22,
}
...
-- // ─────────────────────────────────────────────
-- // STATE-MACHINE FLIGHT (no jitter, no back)
-- // ─────────────────────────────────────────────
local flyConn = nil
local bodyVel  = nil
local bodyGyro = nil
...
-- Single-shot flight. Hard snap on arrival. No oscillation.
local function flyTo(targetPos, speed, onArrived)
    ...
    flyConn = RunService.Heartbeat:Connect(function()
        ...
        if dist <= CFG.APPROACH_DIST then
            arrived = true
            root.CFrame = CFrame.new(targetPos)  -- HARD SNAP, no bounce
...
-- Stable hover: use CFrame lock instead of velocity → zero drift
local hoverConn = nil
...
-- // ─────────────────────────────────────────────
-- // INTERACT BURST (single-frame, no waits)
-- // ─────────────────────────────────────────────
...
-- // ─────────────────────────────────────────────
-- // STEAL — 5 STUDS ABOVE CENTER (TRAP-SAFE)
-- // ─────────────────────────────────────────────
local function stealEgg(egg, center, onDone)
    ...
    -- 5 studs above egg center = trap-safe landing zone
    local hoverPos = eggCenter + Vector3.new(0, CFG.HOVER_ABOVE, 0)

    enableGodMode()

    -- fly directly to hover position (5 studs above)
    flyTo(hoverPos, CFG.FLY_SPEED, function()
        ...
        -- interact burst (3 rapid passes, no delays)
...
-- // ─────────────────────────────────────────────
pcall(function()
    for _,n in ipairs({"BulacatHubV4","BulacatHubV5","BulacatHubV51","BulacatHubV52"}) do
        local o = CoreGui:FindFirstChild(n); if o then o:Destroy() end
    end
end)

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name            = "BulacatHubV52"
...
mk("TextLabel",{Size=UDim2.new(1,0,0,12),Position=UDim2.new(0,0,1,2),BackgroundTransparency=1,Text="BH v5.2",TextSize=8,Font=Enum.Font.GothamBold,TextColor3=C.accentHi,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=43},LogoOuter)
...
mk("TextLabel",{Size=UDim2.new(0,200,0,12),Position=UDim2.new(0,44,0,24),BackgroundTransparency=1,Text="Egg Stealer v5.2 • Smooth",TextColor3=C.subtext,TextSize=9,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=12},Header)
...
local function buildRow(data, idx)
    ...
    mk("Frame",{Size=UDim2.new(0,3,1,-6),Position=UDim2.new(0,3,0,3),BackgroundColor3=rd.color,BorderSizePixel=0,ZIndex=14},row):Clone() -- stripe
...
-- // ─────────────────────────────────────────────
-- // STEAL ONCE — PROPER ONE-SHOT LATCH
-- // Fires ONE steal, then disarms when callback returns.
-- // ─────────────────────────────────────────────
local stealOnceBusy = false       -- currently processing the one-shot
local stealOnceDone = false       -- already completed, waiting for user to re-enable
...
local function runAutoStealTick()
    ...
    if not autoOn then
        -- reset one-shot latch when auto is off
        stealOnceBusy = false
        stealOnceDone = false
        return
    end
    ...
    -- ONE-SHOT MODE: fire once, then disarm
    if onceOn then
        ...
        if ok and eggs and #eggs > 0 then
            ...
            if target then
                ...
                stealEgg(target.egg, target.center, function(success)
                    State.stealing = false
                    stealOnceBusy = false
                    stealOnceDone = true      -- ← latch: won't fire again
                    setAutoOn(false)          -- auto-disarm
                    setOnceOn(false)          -- visually disarm the once toggle
                    setStatus(success and ("✅ [ONCE] "..target.pet) or ("❌ [ONCE] failed"),
    ...
    -- CONTINUOUS MODE (auto on, once off)
    local ok, eggs = pcall(scanEggs)
...
print("🐱 BULACAT HUB v5.2 — Smooth Steal + 5-Stud Hover. Click the logo!")